    ...
    <section id="skills" className="py-24 relative max-w-5xl mx-auto px-6">
      <div className="mb-16 text-center">
        ...
        <p className="text-zinc-400 text-lg max-w-2xl mx-auto">
          A blend of software engineering disciplines, focusing on high-performance backends, AI integration, and modern cloud infrastructure.
        </p>
      </div>
      ...
      >
        ...
        <motion.div variants={item} className="bento-card relative overflow-hidden group flex flex-col p-8">
          ...
          <div className="z-10 mt-6">
            ...
            <p className="text-zinc-400 text-sm">Spring Boot, FastAPI, Node.js, Express. Secure & scalable RESTful APIs.</p>
          </div>
        </motion.div>
        ...
        <motion.div variants={item} className="bento-card relative overflow-hidden group flex flex-col p-8">
          ...
          <div className="z-10 mt-6">
            ...
            <p className="text-zinc-400 text-sm">AWS Lambda, DynamoDB, S3, Docker architectures.</p>
          </div>
        </motion.div>

        {/* Frontend - Large spanning 2 cols */}
        <motion.div variants={item} className="bento-card relative overflow-hidden group md:col-span-2 flex justify-between items-end p-8">
          ...
          <div className="z-10 max-w-sm">
            ...
            <h3 className="text-2xl font-bold text-zinc-100 mb-2">Frontend Interfaces</h3>
          ...
          <div className="z-10 hidden md:grid grid-cols-2 gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
            {["React", "Next.js", "Redux", "Sass"].map((skill) => (
              <div key={skill} className="px-3 py-1 border border-zinc-700 rounded-md text-xs text-zinc-300 backdrop-blur-sm">
                {skill}
              </div>
            ))}
          </div>
        ...
        {/* Core Languages */}
        <motion.div variants={item} className="bento-card md:col-span-3 flex flex-wrap gap-4 items-center justify-center p-6 border-dashed border-zinc-800">
          <Code className="w-5 h-5 text-zinc-500 mr-2" />
          {["Java", "Python", "TypeScript", "JavaScript", "SQL", "PL/SQL"].map((lang) => (
            <span key={lang} className="px-4 py-2 bg-zinc-900/50 rounded-full text-zinc-300 font-medium text-sm border border-zinc-800 hover:border-zinc-700 transition-colors">
              {lang}
            </span>
          ))}
        </motion.div>
        ...
        {/* Databases */}
        <motion.div variants={item} className="bento-card md:col-span-3 flex flex-wrap gap-4 items-center justify-center p-6 bg-zinc-900/20">
          <Database className="w-5 h-5 text-zinc-500 mr-2" />
          {["Oracle Database", "PostgreSQL", "MongoDB", "Redis", "DynamoDB"].map((db) => (
            <span key={db} className="text-zinc-400 font-medium text-sm hover:text-blue-400 transition-colors cursor-default">
              {db}
            </span>
          ))}
        </motion.div>
